/*
  # Update existing database schema for gaming community
  ...
  2. New Features
    - Enhanced pricing structure for games (rental options)
    - Bestseller functionality
...
BEGIN
  ...
  -- Ensure platform column is array type
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'games' AND column_name = 'platform' AND data_type != 'ARRAY') THEN
    -- Convert platform to array if it's not already
    ALTER TABLE games ALTER COLUMN platform TYPE TEXT[] USING ARRAY[platform];
  END IF;
  
  -- Ensure type column is array type
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'games' AND column_name = 'type' AND data_type != 'ARRAY') THEN
    -- Convert type to array if it's not already
    ALTER TABLE games ALTER COLUMN type TYPE TEXT[] USING ARRAY[type];
  END IF;
...
DO $$
BEGIN
  -- Add sample games if games table is empty
  ...
  -- Add sample testimonials if testimonials table is empty
