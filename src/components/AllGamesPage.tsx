...
const AllGamesPage: React.FC<AllGamesPageProps> = ({
  onGameClick,
  onBackToHome,
}) => {
  ...
  useEffect(() => {
    const state = location.state as { search?: string; platform?: string } | null;

    if (state?.search) {
      setSearchQuery(state.search);
      setTempSearchQuery(state.search);
    }
  ...
  const filteredAndSortedGames = useMemo(() => {
    const filtered = games.filter((game) => {
      ...
      const matchesPrice =
        startingPrice >= priceRange[0] && startingPrice <= priceRange[1];
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredAndSortedGames.length / itemsPerPage)
  );
    ...
    return (
      <div className="flex flex-wrap gap-1">
        {platforms.slice(0, 3).map((platform) => (
          <span
            key={platform}
            className="bg-gradient-to-r from-cyan-400 to-blue-500 text-white px-2 py-1 rounded text-xs font-medium shadow-sm"
          >
            {platform}
          </span>
        ))}
  ...
  const showingFrom = filteredAndSortedGames.length === 0 ? 0 : startIndex + 1;
              ...
              <div className="mb-4 sm:mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-orange-500 hover:to-red-500 text-white px-4 py-2 rounded-lg shadow-lg transition-all"
                  >
                    Search
                  </button>
                </div>
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <p className="text-gray-600 text-sm sm:text-base">
                Showing {showingFrom}-{showingTo} of {filteredAndSortedGames.length}{" "}
                games
              </p>
            </div>
                ...
                {paginatedGames.map((game) => (
                  ...
                  >
                    <div className="relative overflow-hidden">
                      ...
                      />
                      <div className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-gradient-to-r from-cyan-400 to-blue-500 text-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-xs font-medium shadow-lg max-w-[70%] truncate">
                        {getPlatformsForGame(game).slice(0, 2).join(", ") || "Game"}
                      </div>
                    </div>
                    ...
                    <div className="p-3 sm:p-4">
                      ...
                      <div className="mb-2">{renderPlatformBadges(game)}</div>
