...
import { Search, Grid, List, Tv } from "lucide-react";
...
const SubscriptionsPage: React.FC = () => {
  ...
  const handleProductClick = (subscription: Game) => {
    ...
    return (
      <div className="flex flex-wrap gap-1.5 mt-2">
        {platforms.map((platform) => (
          <span
            key={platform}
            className="bg-gradient-to-r from-cyan-400 to-blue-500 text-white px-2 py-1 rounded-full text-xs font-medium"
          >
            {platform}
          </span>
        ))}
  ...
  const renderSubscriptionCard = (subscription: Game) => {
    const startingPrice = getStartingPrice(subscription);

    return (
      <div
        key={subscription.id}
        onClick={() => handleProductClick(subscription)}
      ...
      >
        ...
        <div className="p-4 sm:p-5">
          ...
          {renderPlatformBadges(subscription)}
  ...
  const renderSubscriptionListItem = (subscription: Game) => {
    const startingPrice = getStartingPrice(subscription);
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/20 mb-6 sm:mb-8">
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                className="px-4 py-3 rounded-2xl border border-gray-200 bg-white/90 focus:outline-none focus:ring-2 focus:ring-cyan-400"
              >
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <p className="text-sm text-gray-600">
              ...
              subscription{totalCount !== 1 ? "s" : ""} found
            </p>
          </div>
        </div>
          ...
          <>
            {viewMode === "grid" ? (
              ...
              <div className="space-y-4 sm:space-y-6">
                {subscriptions.map((subscription) =>
                  renderSubscriptionListItem(subscription)
                )}
              </div>
            )}
