...
import { useRecommendedGames } from '../hooks/useSupabaseData';
...
const CartModal: React.FC<CartModalProps> = ({ 
  ...
  const { recommendedGames, loading: recommendedLoading } = useRecommendedGames(4);
  ...
  const handleAddRecommendedToCart = (game: Game) => {
    if (onAddToCart) {
      // Use default settings: first platform, Rent type, 1 month duration
      const platform = game.platform[0] || 'PS5';
      const type = 'Rent';
      const price = getGameDisplayPrice(game, type, '1_month');
